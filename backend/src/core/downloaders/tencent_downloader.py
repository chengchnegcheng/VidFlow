...
class TencentDownloader(BaseDownloader):
    ...
    def __init__(self, output_dir: str = None):
        ...
        self.platform_name = "tencent"

    @staticmethod
    def supports_url(url: str) -> bool:
        """检查是否支持该URL"""
        url_lower = url.lower()
        return 'v.qq.com' in url_lower

    async def get_video_info(self, url: str) -> Dict[str, Any]:
        """获取腾讯视频信息"""
        try:
            logger.info(f"[TencentDownloader] get_video_info called for: {url}")
            ...
            if not video_data:
                raise Exception("无法获取视频信息，腾讯视频可能需要登录或该视频不可用")
            ...
            error_str = str(e)
            logger.error(f"Error extracting Tencent video info: {e}")
            ...
            if "Playwright" in error_str:
                ...
                raise Exception(f"获取腾讯视频信息失败: {error_str}")
    ...
    async def _get_video_info_with_playwright(self, url: str, retry_after_install: bool = True) -> Optional[Dict]:
        ...
        if not available:
            if error_code in ("playwright_not_installed", "chromium_not_installed") and retry_after_install:
                logger.info(f"[TencentDownloader] Playwright not available ({error_code}), attempting auto-install...")
                install_result = await auto_install_playwright()

                if install_result.get("success"):
                    logger.info("[TencentDownloader] Playwright auto-installed, retrying...")
        ...
        logger.info(f"[Playwright] Fetching Tencent video info for: {url}")

        # 设置浏览器路径
        browser_exe_path = self._find_browser_executable()

        # 获取 Cookie
        cookie_path = self._get_tencent_cookie_path()
        cookies = []
        ...
        async with async_playwright() as p:
            ...
            async def handle_response(response):
                ...
                # 腾讯视频流 URL 特征
                if any(pattern in url_lower for pattern in ['.m3u8', '.mp4', 'btrace.video.qq.com', 'vd.l.qq.com', 'video.dispatch.tc.qq.com']):
            ...
            try:
                ...
                if not video_urls:
                    try:
                        play_btn = await page.query_selector('.txp_btn_play, .btn_play, [class*="play-btn"]')
                        if play_btn:
    ...
    def _get_tencent_cookie_path(self) -> Optional[Path]:
        """获取腾讯视频 Cookie 文件路径"""
        from .cookie_manager import get_cookie_base_dir
        cookie_dir = get_cookie_base_dir()
        cookie_file = cookie_dir / "tencent_cookies.txt"

        if cookie_file.exists():
            logger.info(f"[TencentDownloader] Found tencent cookie: {cookie_file}")
            return cookie_file
    ...
    async def download_video(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"[TencentDownloader] Downloading video to: {full_output_path}")
            ...
            logger.info(f"[TencentDownloader] Download completed: {full_output_path}")
            ...
            logger.error(f"Error downloading Tencent video: {e}")
            if progress_callback:
                await progress_callback({
                    'task_id': task_id,
                    'status': 'error',
