...
class YoukuDownloader(BaseDownloader):
    ...
    def __init__(self, output_dir: str = None):
        ...
        self.platform_name = "youku"

    @staticmethod
    def supports_url(url: str) -> bool:
        """检查是否支持该URL"""
        url_lower = url.lower()
        return 'youku.com' in url_lower

    async def get_video_info(self, url: str) -> Dict[str, Any]:
        """获取优酷视频信息"""
        try:
            logger.info(f"[YoukuDownloader] get_video_info called for: {url}")
            ...
            if not video_data:
                raise Exception("无法获取视频信息，优酷可能需要登录或该视频不可用")
            ...
            error_str = str(e)
            logger.error(f"Error extracting Youku video info: {e}")
            ...
            if "Playwright" in error_str:
                ...
                raise Exception(f"获取优酷视频信息失败: {error_str}")
    ...
    async def _get_video_info_with_playwright(self, url: str, retry_after_install: bool = True) -> Optional[Dict]:
        ...
        logger.info(f"[YoukuDownloader] Playwright check: available={available}, error_code={error_code}")

        if not available:
            if error_code in ("playwright_not_installed", "chromium_not_installed") and retry_after_install:
                logger.info(f"[YoukuDownloader] Playwright not available ({error_code}), attempting auto-install...")
                install_result = await auto_install_playwright()

                if install_result.get("success"):
                    logger.info("[YoukuDownloader] Playwright auto-installed, retrying...")
        ...
        logger.info(f"[Playwright] Fetching Youku video info for: {url}")

        # 设置浏览器路径
        browser_exe_path = self._find_browser_executable()

        # 获取 Cookie
        cookie_path = self._get_youku_cookie_path()
        cookies = []
        ...
        async with async_playwright() as p:
            ...
            async def handle_response(response):
                ...
                # 优酷视频流 URL 特征
                if any(pattern in url_lower for pattern in ['.m3u8', '.mp4', 'vali.cp31.ott.cibntv.net', 'k.youku.com', 'pl-ali.youku.com']):
            ...
            try:
                ...
                page_info = await page.evaluate('''() => {
                    ...
                    for (const sel of posterSelectors) {
                        ...
                        if (el) {
                            if (el.poster) {
                                result.thumbnail = el.poster;
                                break;
                            } else if (el.src) {
                                result.thumbnail = el.src;
                                break;
                            } else if (el.content) {
                                result.thumbnail = el.content;
                                break;
                            }
                        }
                    }
                    ...
                    for (const sel of descSelectors) {
                        const el = document.querySelector(sel);
                        if (el) {
                            result.description = el.textContent?.trim() || el.content || '';
                    ...
                    for (const sel of uploaderSelectors) {
                        const el = document.querySelector(sel);
                        if (el && el.textContent) {
                            result.uploader = el.textContent.trim();
                            if (result.uploader) break;
                        }
                    }
                ...
                if not video_urls:
                    try:
                        play_btn = await page.query_selector('.play-btn, .kui-play-icon, [class*="play"]')
                        if play_btn:
    ...
    def _get_youku_cookie_path(self) -> Optional[Path]:
        """获取优酷 Cookie 文件路径"""
        from .cookie_manager import get_cookie_base_dir
        cookie_dir = get_cookie_base_dir()
        cookie_file = cookie_dir / "youku_cookies.txt"

        if cookie_file.exists():
            logger.info(f"[YoukuDownloader] Found youku cookie: {cookie_file}")
            return cookie_file
    ...
    async def download_video(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"[YoukuDownloader] Downloading video to: {full_output_path}")
            ...
            logger.info(f"[YoukuDownloader] Download completed: {full_output_path}")
            ...
            logger.error(f"Error downloading Youku video: {e}")
            if progress_callback:
                await progress_callback({
                    'task_id': task_id,
                    'status': 'error',
